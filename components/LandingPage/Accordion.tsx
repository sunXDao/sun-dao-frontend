...
export default function Accordion({ question, answer, handleOnClick, open }: { question: string, answer: string, handleOnClick: () => void, open: boolean }) {
  return (
    <div className='text-white'>
      <div onClick={handleOnClick} className="flex justify-between gap-2 rounded-2xl p-5 border border-[#85A09B] border-opacity-50 cursor-pointer">
        <h2 className='font-bold text-xl'>{question}</h2>
        <div className='cursor-pointer h-fit'>
          <BiChevronDown className={`transition-all ease-in-out ${open ? 'rotate-180' : 'rotate-0'} duration-500 h-8 w-8`} />
        </div>
      </div>
